...
async function fetchContent() {
  try {
    ...
    const guardianApiUrl = `https://content.guardianapis.com/search?api-key=${process.env.GUARDIAN_API_KEY}&show-fields=all`;

    // Add error handling for each API request
    let newsResponse, guardianResponse, healthData;
    
    try {
      [newsResponse, guardianResponse, healthData] = await Promise.all([
      ...
      // If Guardian API fails, try to continue with just news API
      if (!guardianResponse) {
        [newsResponse, healthData] = await Promise.all([
          axios.get(newsApiUrl),
          fetchHealthData(),
        ]);
      }
    ...
    // Add News API articles if available
    if (newsResponse && newsResponse.data && newsResponse.data.articles) {
      allArticles = [...allArticles, ...newsResponse.data.articles];
    }
    
    // Add Guardian API articles if available
    if (guardianResponse && guardianResponse.data && guardianResponse.data.response && guardianResponse.data.response.results) {
      allArticles = [...allArticles, ...guardianResponse.data.response.results];
    }
    
    // If no articles are available, return null
    if (allArticles.length === 0) {
      console.error("No articles available from either API");
      return null;
    }
    ...
    // Safely extract article properties with fallbacks
    const isGuardianArticle = 'webTitle' in randomArticle;
    
    return {
      article: {
        title: isGuardianArticle ? randomArticle.webTitle : (randomArticle.title || "Today's News"),
        description: isGuardianArticle 
          ? (randomArticle.fields?.bodyText?.substring(0, 200) || "Read the full article for more details.") 
          : (randomArticle.description || "Read the full article for more details."),
        url: isGuardianArticle ? randomArticle.webUrl : (randomArticle.url || ""),
        image: isGuardianArticle ? randomArticle.fields?.thumbnail : (randomArticle.urlToImage || ""),
      },
      healthData,
    };
  } catch (error) {
